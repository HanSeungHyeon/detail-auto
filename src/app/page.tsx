    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 transition-colors duration-300">
      <header className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-xl border-b border-border transition-colors duration-300">
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          <div className="font-bold text-xl tracking-tight flex items-center gap-2">
            ...
            <span className="text-foreground font-bold text-xl tracking-wide">DA</span>
          </div>
          <nav className="flex items-center gap-3">
            ...
            {!isChecking && (
               isAuthenticated ? (
                <Link href="/dashboard">
                  <Button className="bg-foreground text-background hover:bg-foreground/90 rounded-2xl px-6 font-semibold shadow-sm transition-transform active:scale-95">
                    대시보드로 이동
                  </Button>
                </Link>
              ) : (
                <>
                  <Link href="/login">
                    <Button variant="ghost" className="text-muted-foreground hover:bg-accent hover:text-foreground font-medium rounded-xl transition-colors">
                      로그인
                    </Button>
                  </Link>
                  <Link href="/signup">
      ...
      <main className="pt-40 pb-32 container mx-auto px-4">
        ...
        <section className="max-w-4xl mx-auto text-center space-y-10 mb-32 animate-in fade-in slide-in-from-bottom-12 duration-1000">
           <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-blue-50/10 text-blue-600 dark:text-blue-400 text-[15px] font-bold mb-4 shadow-sm border border-blue-100/20">
            <span className="flex h-2.5 w-2.5 rounded-full bg-blue-600 dark:bg-blue-400 animate-pulse"></span>
            첫 상세페이지 무료 제작 이벤트
          </div>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-foreground leading-[1.05]">
            이미지 한 장으로<br />
            <span className="text-primary">압도적 상세페이지</span>
          </h1>
          <p className="text-xl md:text-2xl text-muted-foreground whitespace-pre-line leading-relaxed max-w-2xl mx-auto font-semibold tracking-tight">
            사진과 타겟 정보만 입력하세요.{"\n"}
        ...
        <section id="sample-section" className="pt-32 border-t border-border max-w-6xl mx-auto scroll-mt-24">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-10">
            {sampleProducts.map((sample) => (
               <Link key={sample.id} href={`/sample/${sample.id}`} className="group block">
                <div className="bg-card rounded-[32px] overflow-hidden border border-border shadow-sm hover:border-primary/50 hover:shadow-xl transition-all duration-500 hover:-translate-y-2 flex flex-col sm:flex-row h-full">
                  <div className="sm:w-1/2 aspect-[4/3] sm:aspect-auto relative overflow-hidden bg-muted shrink-0">
                    ...
                    <div className="absolute top-5 left-5 px-4 py-1.5 bg-background/90 backdrop-blur-md text-primary text-[14px] font-black rounded-full shadow-lg border border-border">
                  ...
                  <div className="p-10 flex-1 flex flex-col justify-center">
                    <h3 className="font-black text-[24px] text-foreground mb-4 group-hover:text-primary transition-colors line-clamp-2 leading-tight tracking-tight">{sample.title}</h3>
                    <p className="text-[16px] font-semibold text-muted-foreground line-clamp-3 leading-relaxed tracking-tight">{sample.subtitle}</p>
                  </div>
                </div>
              </Link>
            ))}
